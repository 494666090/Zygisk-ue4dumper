...
bool Package::DumpFunctions(std::string path) const
{
	...
	for (auto& curClass : classes)
	{
		for (auto& curMethod : curClass.Methods)
		{
			...
			auto fnName = tfm::format("%s::%s", curClass.NameCpp, curMethod.Name);

			tfm::format(o, "%p , %-100s \"%s\" \n", curMethod.NativeFunc, fnName, BuildMethodSignature(curMethod, curClass, false));
		}
	}
...
void Package::GenerateScriptStruct(const UEScriptStruct& scriptStructObj)
{
	...
	{
		offset = super.GetPropertySize();

		if(super.GetName() == "Object")
		{
			offset -= sizeof(uint32_t);
		}
		ss.InheritedSize = offset;
...
std::string Package::BuildMethodBody(const Class& c, const Method& m) const
{
	...
	if (generator->ShouldConvertStaticMethods())
	{
